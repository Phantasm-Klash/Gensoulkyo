...
## 2026-07-01 nakama-server-agent migration registry update

- Added PostgreSQL draft targets for `battle_servers` registry state and `packet_key_versions` rotation metadata, including dev scaffold key-version seeds for business envelopes and battle tickets.
- Kept the change persistence-only: battle server lifecycle audit rows still flow through existing allocation audit repository paths, and Go/Nakama still does not own high-frequency battle ticks or client-authored results.
