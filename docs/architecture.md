...
- `runtime/httpapi`: HTTP routes that expose the core service using the same payload shapes the Godot client prototype already models, including battle server lifecycle, battle allocation, ticket fallback and consume routes, battle result submit fallback route, optional `X-PhK-Business-*` envelope checks, and development status endpoints for the envelope guard plus battle/lobby lifecycle audit repositories.
...
- Matched queues create a server-owned battle allocation with battle server id, endpoint, per-player battle ids, deck snapshot hashes, mode config hash, seed, and protocol version. Each participant can receive a short-lived Ed25519-signed battle ticket bound to match id, user id, player id, deck hash, ruleset, endpoint, and a non-login business session reference; the raw bearer session token is not embedded in the ticket. Battle service callbacks can mark tickets consumed by id/match/server/nonce, and duplicate consume callbacks are idempotent.
- When multiple battle servers are healthy at the same computed load, allocation prefers the most mode-specific server over broad fallback servers. Offline or stale registered battle servers are skipped for new allocations, keeping the local all-mode development server available while allowing a registered PvP/Boss C++ battle server to take its intended traffic.
- Battle result submission is a service-side contract, not a client-authoritative result path. The current development route validates `SignedBattleResult` protocol, business API, battle API, and ruleset version stamp, match id, mode id, allocation player ids, result hash, replay id, settled time, battle server key id, Ed25519 signature field shape, and idempotent replay before reusing the existing server settlement path. Attributable rejected callbacks are persisted as rejected battle-result audits with a reason code, without settling the match or writing replay audits. Real signature verification, mTLS, protobuf decoding, and C++ service callbacks are still production work.
...
- `business.contract` is the SDK-neutral Nakama RPC/WSS read for the shared low-frequency business transport, notification topic, service callback, authority, and forbidden-field contract before a client has joined a room. `business.event` is the matching WSS/status payload contract for low-frequency business updates such as queue progress, room snapshots, matchmaking found, ready status, battle allocation, signed battle ticket delivery, and settlement notices. `BusinessContractSnapshot`, `RoomRulesSnapshot`, and `BusinessEvent` all expose `business_notifications` and `business_event_request_kinds`, and each notification topic publishes its fixed read operation plus `client_request_authority=lookup_only`, so clients can distinguish low-frequency business WSS reads from service-origin callbacks; high-frequency battle ticks and client result submission remain disallowed on the business channel.
...
- Production Nakama status/socket publication on top of the current `business.event`, heartbeat, event-polling, allocation, and ticket fallback contracts.
- Persistent stores beyond the audit repositories, including sessions/users, inventory, decks, chests, rewards, activities, leaderboards, replay objects, and world Boss state.
