...
- `business.event` is the SDK-neutral Nakama WSS/status payload contract for low-frequency business updates such as queue progress, room snapshots, matchmaking found, ready status, battle allocation, and signed battle ticket delivery. It explicitly reports that high-frequency battle ticks are not allowed on the business channel, that client result submission is not a client operation, that the business envelope is required, and which client-authored authority fields are forbidden.
