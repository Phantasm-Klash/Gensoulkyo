...
- `business.event` is the SDK-neutral Nakama WSS/status payload contract for low-frequency business updates such as queue progress, room snapshots, matchmaking found, ready status, battle allocation, signed battle ticket delivery, and settlement notices. `RoomRulesSnapshot` and `BusinessEvent` both expose `business_notifications` so clients can distinguish low-frequency business WSS topics from service-origin callbacks; high-frequency battle ticks and client result submission remain disallowed on the business channel.
