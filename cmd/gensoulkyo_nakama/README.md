...
`battle.servers.register`, `battle.servers.heartbeat`, and `battle.servers.offline` are registered for service-to-service battle server lifecycle callbacks. The binding marks them as service-origin only for the explicit callback allowlist, only when Nakama supplies no player `session_id`/`user_id`, and only when `runtime.RUNTIME_CTX_MODE` is present and not `client`. Player-scoped calls therefore fail closed in `runtime/nakamaapi`; authenticated clients should only use `battle.servers` for business-envelope-protected discovery.

`battle.result.submit` is registered for service-to-service C++ Battle Server callbacks under the same allowlist and context gate. Public player calls still fail before core result validation, so clients cannot use the RPC as an authority path for damage, rewards, or settlement.
